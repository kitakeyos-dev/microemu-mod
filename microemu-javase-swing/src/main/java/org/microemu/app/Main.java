...
import me.kitakeyos.plugin.reload.HotReloadConfig;
import me.kitakeyos.plugin.reload.HotReloadManager;
import me.kitakeyos.plugin.updater.PluginUpdateManager;
import me.kitakeyos.script.ui.LuaScriptManager;
import org.microemu.*;
...
public class Main extends JFrame {
    ...
    public static URIMapperGUI uriMapperGUI;
    ...
    protected EmulatorContext emulatorContext = new EmulatorContext() {
        ...
        public InputStream getResourceAsStream(String name) {
            return Objects.requireNonNull(MIDletBridge.getCurrentMIDlet()).getClass().getResourceAsStream(name);
        }
    ...
    private ActionListener menuAboutListener = e -> SwingDialogWindow.show(Main.this, "About", new SwingAboutDialog(), false);
    ...
    private ActionListener menuScaledDisplayListener = new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (selectedZoomLevelMenuItem.isSelected()) {
                ...
                updateScaledImageListener = image -> {
    ...
    private HotReloadManager reloadManager;
    ...
    public Main(DeviceEntry defaultDevice) {
        ...
        JCheckBoxMenuItem luaScriptItem = new JCheckBoxMenuItem("Lua Script");
        SwingUtilities.invokeLater(() -> luaScriptManager = new LuaScriptManager(luaScriptItem));
        luaScriptItem.addActionListener(e -> {
            boolean selected = luaScriptItem.isSelected();
            luaScriptManager.setVisible(selected);
        });
        tools.add(luaScriptItem);

        JCheckBoxMenuItem uriMapperItem = new JCheckBoxMenuItem("URI Mapper");
        SwingUtilities.invokeLater(() -> uriMapperGUI = new URIMapperGUI(luaScriptItem));
        uriMapperItem.addActionListener(e -> {
            boolean selected = uriMapperItem.isSelected();
            uriMapperGUI.setVisible(selected);
        });
        tools.add(uriMapperItem);

        menuOptions.add(tools);
        ...
        menuShowMouseCoordinates.addActionListener(event -> devicePanel.switchShowMouseCoordinates());
        ...
        this.resizeButton.addActionListener(ev -> {
        ...
        File pluginUpdateDir = new File("plugin-updates");
        pluginManager = new PluginManager(pluginDir, eventBus);
        PluginUpdateManager updateManager = new PluginUpdateManager(pluginDir, pluginUpdateDir, pluginManager.getLoader());

        updateManager.applyAllUpdates();
    ...
    private void setupHotReload() {
        ...
        reloadManager = new HotReloadManager(pluginManager, config);
